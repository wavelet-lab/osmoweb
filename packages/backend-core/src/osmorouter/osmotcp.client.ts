...
export class OsmoTcpClient {
    ...
    constructor(params: ServiceAddr, name: string = '', logger?: LoggerInterface) {
        this._name = name;
        this.logger = logger ?? new SimpleLogger(OsmoTcpClient.name);
    ...
    onConnect() {
        this.logger.log(`OsmoTcpClient(${this.name}).onConnect: connection to ${this.uri}:${this.port} established`)
        this._connected = true;
    }

    onDisconnect() {
        this.logger.log(`OsmoTcpClient(${this.name}).onDisconnect: disconnected from ${this.uri}:${this.port}`)
        this._connected = false;
    }

    onDataInternal(data: Buffer) {
        this.logger.debug?.(`OsmoTcpClient(${this.name}).onData: Data received from server: ${data.toString('hex')}`)
        this.logger.debug?.(`OsmoTcpClient(${this.name}).onData: Received ${data.byteLength} bytes`)
        this.circularBuffer.push_back(data);
        if (this.onData) this.onData(data);
    }

    onError(err: Error) {
        this.logger.error(`OsmoTcpClient(${this.name}).onError: ${err}`)
    }

    send(data: Buffer) {
        this.client.write(data, (error) => {
            if (error) {
                this.logger.error(`OsmoTcpClient(${this.name}).sendData: ${error}`)
                this.disconnect();
            }/*  else {
